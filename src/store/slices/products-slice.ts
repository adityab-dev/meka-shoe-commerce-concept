...
const productsSlice = createSlice({
  ...
  reducers: {
    productAddedToCart(state, action: PayloadAction<number>) {
      ...
      console.log("entered productAddedTocart");

      let indexOfProduct = 0;

      indexOfProduct = state.products.findIndex(
        (product) => product.id === idOfProduct
      );

      if (!(state.products[indexOfProduct].quantity === 0)) {
        state.products[indexOfProduct].quantity -= 1;
      }

      const filteredProducts = state.filteredProducts;

      indexOfProduct = filteredProducts.findIndex(
        (product) => product.id === idOfProduct
      );

      if (!(state.filteredProducts[indexOfProduct].quantity === 0))
    ...
    enterDataOnFilterField(
      ...
      const isValidStateKey =
        name === brands || name === colors || name === sizes;
    ...
    checkboxInteraction(
      ...
      if (isValidStateKey && !checked) {
        ...
        indexOfItem = state.appliedFilters[name].findIndex(
          (el) => el === value
        );
    ...
    onFiltersChange(state) {
      ...
      const filteredColors = products.filter((product) => {
        for (let color of uniqueColors) {
          for (let productColor of product.colors) {
            if (productColor.toLowerCase().includes(color.toLowerCase()))
              return product;
          }
        }
        return null;
      ...
      if (!filteredBrands.length && !filteredColors.length)
        filter_brands_colors = products;
      ...
      if (!filteredSizes.length)
        filter_brands_colors_sizes = filter_brands_colors;
      ...
      if (price === "0")
        filter_brands_colors_sizes_price = filter_brands_colors_sizes;
      else
        for (let product of filter_brands_colors_sizes_price) {
